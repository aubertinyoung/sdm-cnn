...
class OccurrenceSeries(gpd.GeoSeries):
...
class OccurrenceFrame(gpd.GeoDataFrame):
    ...
    def _constructor_sliced(self):
        def _geodataframe_constructor_sliced(*args, **kwargs):
            ...
            if isinstance(getattr(srs, "dtype", None), gpd.base.BaseGeometry) and not is_row_proxy:
                srs = OccurrenceSeries(srs)
            return srs
    ...
    def within(
        self,
        raster: xarray.DataArray) -> OccurrenceFrame:
        """
    ...
    #     minimum: Optional[int] = None) -> OccurrenceFrame:
    #     """
    ...
    #     buffered_gdf = self._obj.buffer(distance/2, cap_style=3)
